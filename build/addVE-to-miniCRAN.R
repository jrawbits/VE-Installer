...
# Remember to have ve.miniCRAN into .RProfile or otherwise have it set when running this script
...
miniCRAN::addLocalPackage(dir(dir.src),pkgPath=dir.src,path=ve.miniCRAN,type="source")
...
if ( file.exists(dir.bin) && length(dir(dir.bin))>0 ) {
    miniCRAN::addLocalPackage(dir(dir.bin),pkgPath=dir.bin,path=ve.miniCRAN,type="win.binary")
} else {
