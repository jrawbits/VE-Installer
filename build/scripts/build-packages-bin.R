...
package.names <- pkgs.visioneval[,"Package"]
package.paths <- file.path(pkgs.visioneval[,"Root"], pkgs.visioneval[,"Path"], package.names)
...
if ( build.type == "win.binary" ) {
  for ( module in package.paths ) {
    # Use arcane bypass via devtools to save the .zip generated by
    # R CMD INSTALL in the right place
    if ( ! moduleExists(module, built.path.binary) ) {
      built.package <- devtools::build(module, path=built.path.binary, binary=TRUE, args="--clean")
    } else {
      built.package <- file.path(built.path.binary, modulePath(module, built.path.binary))
    }
    install.packages(built.package, repos=NULL, lib=ve.lib, type="win.binary") # so they will be available for later modules
  }
  ...
  package.names <- setdiff( package.names, pkgs.installed )
  if ( length(package.names)> 0 ) {
    cat("Installing missing packages.\n")
    for ( module in package.names ) {
      install.packages(module, repos=ve.repo.url, lib=ve.lib, type="source")
    }
  } else {
