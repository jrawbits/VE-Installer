# This script completes the VE repository build process

# Any external or visioneval packages that were built into the
# VE repository directories will be added to the PACKAGES files.
...
if ( dir.exists(src.contrib) ) {
	...
	cat("VE repository does not contain a source tree.\n")
	stop("Re-run build-repository.R")
}
if ( dir.exists(bin.contrib) ) {
	num.bin <- write_PACKAGES(bin.contrib,type="win.binary")
} else {
	cat("VE repository does not contain a win.binary tree.\n")
	stop("Re-run build-repository.R")
}
